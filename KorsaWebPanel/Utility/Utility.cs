...
using KorsaWebPanel.Areas.Dashboard.Models;
...
namespace BasketWebPanel
{
    public static class ExtensionMethods
    {
        ...
        public static string GetReportUserTypeStatusText(ReportUserStatusTypes type)
        {
            string returnType = string.Empty;
            switch (type)
            {
                case ReportUserStatusTypes.Spam:
                    returnType = "Spam";
                    break;
                case ReportUserStatusTypes.AbusiveHateful:
                    returnType = "Abusive & Hateful";
                    break;
                default:
    ...
    class MyRegularExpressions
    {
        ...
        public const string Url = @"[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)";
    }
    ...
    public class Utility
    {
        ...
        public static SelectList GetDurationTypeOptions()
        {
            try
            {
                List<SelectListItem> options = new List<SelectListItem>();

                foreach (DurationType type in Enum.GetValues(typeof(PaymentMethods)))
                {
                    options.Add(new SelectListItem { Text = type.ToString(), Value = (((int)type)).ToString() });
                }
        ...
        public static string GetRequestStatusBadge(int Status)
        {
            switch (Status)
            {
                case (int)RequestItemStatus.Requested:
                    return " <span class='label label-default'>Requested</span>";
                case (int)RequestItemStatus.Pending:
                    return "<span class='label label-primary'>Pending</span>";
                case (int)RequestItemStatus.InProgress:
                    return "<span class='label label-primary'>InProgress</span>";
                case (int)RequestItemStatus.Delivered:
                    return "<span class='label label-success'>Delivered</span>";
                case (int)RequestItemStatus.Completed:
                    return "<span class='label label-success'>Completed</span>";
                case (int)RequestItemStatus.Cancelled:
        ...
        public static SelectList GetDriverRequestStatusOptions(string DefaultName = "")
        {
            try
            {
                List<SelectListItem> options = new List<SelectListItem>();

                options.Add(new SelectListItem { Text = "Accept Driver Request", Value = "true" });
                options.Add(new SelectListItem { Text = "Reject Driver Request", Value = "false" });
        ...
        public static SelectList GetLanguageOptions(string DefaultOption = "")
        {
            try
            {
                List<SelectListItem> options = new List<SelectListItem>();

                options.Add(new SelectListItem { Text = "English", Value = "0" });
                options.Add(new SelectListItem { Text = "Arabic", Value = "1" });
        ...
        public static SelectList GetCategoryOptions(IPrincipal User, string DefaultName = "", int? CatId = null)
        {
            try
            {
                var response = AsyncHelpers.RunSync<JObject>(() => ApiCall.CallApi("api/GetAllCategories", User, GetRequest: true, parameters: "IsAdmin=True"));
